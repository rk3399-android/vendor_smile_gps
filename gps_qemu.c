...
gps_state_init( GpsState*  state, GpsCallbacks* callbacks )
{
    ...
    if (state->fd < 0) {
        ALOGE("%s: socket() failed, mSockFd=%d, errno=%d", __FUNCTION__, state->fd, errno);
    }
    ...
    retVal = bind(state->fd, &servAddr, sizeof(servAddr));
    if(retVal < 0) {
    ...
    fcntl(state->fd, F_SETFL, O_NONBLOCK);
    
    int cSockFd = accept(state->fd, &cliAddr, &cliLen);
    if (cSockFd >= 0) {
